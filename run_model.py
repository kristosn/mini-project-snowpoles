...
def write_to_file(dataset, model, imgsz, time_elapsed_train, file):
    IDUN = Path("/cluster").exists()
    with open(file, "a") as f:
        f.write(f"Dataset: {dataset}, Model: {model}, imgsz: {imgsz}, training time (s): {time_elapsed_train:.2f}, IDUN: {IDUN}\n")
...
def run_yolo(dataset, model_path, imgsz, epochs):
    ...
    predict_yolo(best_model_path, PATH_TO_TEST_SET, project, f"predict_yolo_{dataset}_{imgsz}", imgsz)
    write_to_file(dataset, model_path.replace('.pt', ''), imgsz, time_elapsed_train_yolo, "time.txt")
...
def run_rf_detr(dataset, model_name, epochs, resolution):
    ...
    predict_rf_detr(best_model_path, model_name, dataset, PATH_TO_TEST_SET, resolution, threshold=0.5)
    write_to_file(dataset, model_name, resolution, time_elapsed_train, "time.txt")